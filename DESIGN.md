...
- Evolution Chain Visual resolved evolution edges per species, not per form — any EVO_TREE row for a dexId suppressed that species' flat evolution edge entirely, regardless of which form the row actually belonged to; regional-form branches (Alolan, Galarian) leaked onto base-form nodes across ~27 species (e.g. displayed "Rattata → Alolan Raticate") — fixed v0.45, see "Per-Form Edge Resolution & Discovery Gating" under Evolution Chain Visual
- `familyId` split Nidoran into two families (12 and 13) because `FullFetcher.html`'s merge only remapped Nidoran♀'s own row, missing Nidorino/Nidoking on Nidoran♂'s separate PokéAPI chain — fixed v0.45 in both the spreadsheet (one-time renumber) and the fetcher tool (chain-ID-based merge, applies going forward); true family count confirmed as 540, not 549 (549 was the highest chain ID in use, not a count) or 541 (pre-fix, with Nidoran split)
- `state.trainerUnlocks` had existed as a dead, never-wired-up stub since v0.19 (flagged as such in this doc at the time) and was still being read, always-false, by four live call sites with no live writer anywhere in the code — the map's connection-coloring check, `buildTravelPath()`, the reachability BFS behind the mission-destination picker, and `getMissingItemGatesForLocation()`. Found scoping v0.45's map work (initially only the first was noticed), then swept for and removed from all four once confirmed dead — v0.45, confirmed dead throughout
- `state.formsDiscovered` (v0.45's per-form discovery record) was only ever populated via its one-time pre-v0.45-save backfill, which is correctly skipped for a save that already has the field — meaning a normal reload of an already-v0.45 save never restored it at all, silently reverting to empty for the rest of that session and, once autosave fired, in the save file itself — fixed v0.45.1, `loadGame()` now unconditionally restores the field first; see SAVE_VERSION 31's row for the accompanying self-heal
- `evoMethodLabel()`'s `use-item` branch only ever tried `getItem()` for a numeric `e.item`, so a valid string `itemId` slug (most of `evotree.js`'s pre-existing branched rows, e.g. `"leaf-stone"`) fell through to raw, unformatted display instead of resolving to its real item name — fixed v0.45.2, see "Evolution Item Display" under Evolution Chain Visual
- `applyEvolutionSilent()` (offline/batch processing's counterpart to `checkEvolution()`) carried its own copy of the Path 2 branch filter that was missing the v0.42 `fromFormName`/`requiredGender` restriction entirely — every species with a form-branch in EVO_TREE could randomly evolve into the wrong form's target when the level-up happened offline instead of live; reported live as a base-form Meowth evolving into Perrserker — fixed v0.45.2, see "`fromFormName` and `requiredGender` Branch Restrictions" under Branching Evolutions
- Evolution Chain Visual's per-line list (v0.45) derived which forms to render from which forms had an outgoing edge, silently dropping any non-evolving form entirely rather than gating it by discovery — reported live as Farfetch'd's base form never appearing at all (only its evolving Galarian form's line rendered) — fixed v0.45.3, see "Per-Form Edge Resolution & Discovery Gating" under Evolution Chain Visual
- The Unconfirmed-Predecessor-via-Breeding placeholder (v0.36) never checked whether its own root was actually discovered, so a species whose evolution was confirmed without ever personally owning the pre-evolution itself (branch confirmed via testing, base species never caught) rendered a redundant, nonsensical second leading "?" — reported live on Cubone — fixed v0.45.3, see "Unconfirmed Predecessor via Breeding"
- Day Care Research Mode's auto-switch (v0.44) let an entire elapsed real-time backlog accumulate on whichever pair was active before ever checking for a switch, and even once a switch fired, allowed at most one per actual function call regardless of how much real time that call covered — a browser-throttled backgrounded tab could report a large total egg count concentrated on very few distinct species instead of the many a live, unthrottled session would have produced. Reported live: 32 eggs collected, only ~8 families newly researched from one idle-offscreen session — fixed v0.45.3, see "Day Care Research Mode"
- `state.researchLog[dexId]` was one shared record across every form of a dexId, but `professorAutoTestEvolutions()` only marked an item "fully tested" once every branch sharing it — across all forms — resolved. A species with the same item split across a base branch and a form-gated branch (e.g. Growlithe: base + Hisuian, both Fire Stone) could confirm the base branch while the Hisuian one stayed permanently unresolvable (no owned Hisuian Growlithe), so the item never reached `testedMethods` even after `confirmedBranches` already had it — `buildEvoMethodsHtml()` builds its ✓ and ? buckets independently, so the item rendered in both at once. Reported live: Growlithe's Fire Stone showing simultaneously Confirmed and Unknown — fixed v0.45.4, see "Research Log — Per-Form Identity"
- Same root cause, different symptom: a branch restricted to a form the player doesn't own (Vulpix's Ice Stone, `fromFormName:'Alolan'`) could never produce a matching candidate, so it could never resolve to either Confirmed or Ruled Out — indistinguishable from "not tested yet," with every other item on the same species correctly reaching one of those two states. Reported live: Vulpix's Ice Stone stuck at `?` with 49/50 other methods Ruled Out — fixed v0.45.4 (per-form research means Ice Stone isn't even in base Vulpix's own branch list, so it's ruled out immediately and cleanly instead)
- `evotree.js` had 3 `use-item` rows using the item's display name instead of its `itemId` slug (Vulpix and Growlithe base rows: `"Fire Stone"` instead of `"fire-stone"`; Graveler base row: `"Link Cable"` instead of `"link-cable"`) — `professorAutoTestEvolutions()` matches by slug only, so these branches were invisible to the auto-test system regardless of any other fix; a new save's base Growlithe could never confirm Fire Stone at all. Data-only, corrected directly in the spreadsheet and `evotree.js` — v0.45.4
...
**Canonical current version (SETTLED — v0.45.4, NEW).** GitHub `main` (`snipesmcgeee/pokeprof`) is the source of truth for which version of `pokeprof.html`/`DESIGN.md` is actually current — not whatever copy happens to be sitting in a given chat's uploads. A file delivered in one session/chat is not automatically available to a different session working on this project; if there's any doubt about whether an uploaded or locally-held file is current, check `GAME_VERSION`/`SAVE_VERSION` against `main` before building on it. Surfaced when a Claude Code session investigating a bug report was given a stale local v0.44.4 copy despite v0.45.3 having already shipped and (presumably) been pushed.

**`FullFetcher.html` — family-ID (`familyId`) derivation (SETTLED — v0.45).** `familyId` on every `pokedex.js` row is derived from PokéAPI's own evolution-chain ID, normalized to a contiguous 1–540 range:
- **Nidoran merge:** PokéAPI assigns Nidoran♀'s line chain 12 and Nidoran♂'s line (Nidorino, Nidoking) chain 13 — two chains for one family. The merge (chain 13 → 12) is applied via a single function wherever the tool reads a chain ID, for base and variant rows alike, with no dependency on fetch order or session cache (the pre-v0.45 tool's dexId-keyed merge only fixed Nidoran♂ itself and silently missed Nidorino/Nidoking, and did nothing at all unless Nidoran♀ happened to be fetched earlier in the same session).
- **Contiguous renumbering:** PokéAPI's chain-ID space has 9 permanently empty numbers — 13 (vacated by the Nidoran merge above), 210, 222, 225, 226, 227, 231, 238, 251. A pulled species' `familyId` is `chainId` minus the count of these 9 numbers below it (Nidoran-merge case handled first). This keeps existing families' relative order intact and makes `familyId` land in a contiguous 1–540 range instead of 1–549 with gaps — 540 is the true family count as of v0.45 (verified by an independent structural walk of every evolution edge across all 1,025 species, cross-checked against Jack's pre-existing count).
- **Guards:**
  - **Empty-slot check:** if a pulled species' raw chain ID falls in one of the 9 empty slots above, the row is flagged for manual review instead of silently numbered.
  - **Skipped-ID warning:** after any pull, if the newly-assigned family numbers (541+) aren't contiguous, the tool warns that PokéAPI has retired or skipped a new chain ID.
  - **Load-time self-check:** on page load, the derivation function is checked against fixed anchor pairs (1→1, 13→12, 14→13, 209→208, 211→209, 549→540, 550→541) — a red banner appears if any fail.
- **Maintenance rule:** the list of 9 empty numbers is a constant in the tool; if PokéAPI ever retires or introduces a new chain ID, that number is added to the list by hand — this derivation cannot detect a newly-empty or newly-occupied slot on its own, only the skipped-ID warning's downstream symptom.
- **Limit:** the tool cannot detect PokéAPI renumbering an *existing* chain ID out from under a species already in the sheet — that would surface as a mismatch against Jack's existing data on a re-pull, not as a warning from this logic.
...
- **Current save version: 31.** v0.45.4 requires **32** (see table).
...
| v0.45 | 30→31 | Per-species-and-form ownership record (`state.formsDiscovered` or equivalent, keyed by dexId+formName, base form included) — powers the Evolution Chain Visual's per-form discovery gating (see below) | A single helper called from every `dexHistory`-increment site backfills it: existing saves seed the base form as discovered for any species with `dexHistory>0`, plus each currently-held individual's own form. Regional/alt forms owned and released or evolved away before v0.45 aren't recoverable — this is a floor derived from present state, not a full history reconstruction. **v0.45.1 fix:** the field was only ever being restored via this backfill path, which is (correctly) skipped once a save already has the field — meaning a normal reload of an already-v0.45 save never restored it at all, silently reverting to empty for the rest of that session (and into the save file itself, on the next autosave). `loadGame()` now unconditionally restores `state.formsDiscovered = s.formsDiscovered \|\| {}` first; the backfill trigger is also broadened from "field missing" to "field missing or empty," so a save already damaged by this bug self-heals from `dexHistory` on its next load, the same floor as a genuine pre-v0.45 save | Must run after any `dexHistory`/`dex` migration from earlier rows completes, since it reads both |
| v0.45.4 | 31→32 | `state.researchLog` re-keyed from bare `dexId` to `dexId+'\|'+(formName\|\|'')` — every research field (`confirmedBranches`, `testedMethods`, `evolutionConfirmed`, `abilitiesObserved`, `gendersObserved`, `breedingTested`, `highestLevelObserved`, `firstSeen`, `firstCaught`, `knownSince`) now belongs to one specific form, not the species as a whole. See "Research Log — Per-Form Identity" under Research & Pokédex | Re-keying happens immediately when `researchLog` is read from the save, before any other migration touches it: every existing entry moves to that dexId's base-form key (`formName:null`). There is no way to recover which specific form an old aggregate record actually came from, so this is a deliberate, one-time "start the base form where it left off, any other form you already own starts fresh" choice, not a best-effort reconstruction. Any other form you already own gets a normal empty record, populated on its own next natural trigger (ability/gender backfill sweep, method rule-out sweep, or the Professor auto-test loop) — no separate backfill code needed for that. | Must complete before any of the 4 existing migration blocks that read `researchLog` by key run: v0.20 schema migration, v0.26 item-rule-out cleanup, v0.44 `shed` backfill, and the `data.version<21` confirmation-wipe — all 4 updated to parse the compound key (`split('\|')`) instead of assuming a bare dexId string. The two of these that run unconditionally on every load (v0.26, v0.44) would otherwise silently corrupt every load after the first, since `Number("58\|")` is `NaN` |
...
- **v0.45 — third metric, "🧭 Wander (random)".** `state.wanderMetric` gains a third value, `'random'`. Selection draws uniformly at random from the identical eligible pool the other two metrics use (`getWanderReachableLocations()` filtered to `buildRouteTable(locId,...).length>0`) — no new eligibility rule, no new helper function beyond the random pick itself. Reroll happens on the exact same trigger as the other two metrics (`evaluateWanderTarget()`'s stable-at-destination check) — a mid-journey call is a no-op for Random exactly as it is for Encounters/Catches. **No SAVE_VERSION impact** — `'random'` is just a third valid string for a field that already exists (22, unchanged). **Implementation Note:** the existing redirect-to-self guard (`if(best===aide.missionDestination) return`) is intentionally left in place for Random too, even though a genuinely independent roll could legitimately reroll to the same spot the aide just left — this means Random will sometimes silently keep the current destination on a self-matching roll rather than "restarting" the dwell timer. This is accepted behavior, not a bug: reusing the existing guard was judged not worth a metric-specific carve-out.
...
`updateResearchModePairs()`, called once per aide at the top of `gameTick()` alongside `professorAutoTestEvolutions()`: for every aide with `researchMode` and no active `researchPair`, attempts selection. **v0.45.3 fix (behavior corrected from the original description below):** while an aide with an active `researchPair` is away from the Day Care, its egg queue still advances via the same remainder-preserving interval loop `updateDaycareQueue()` uses, uncapped — the clock never pauses. But while the aide **is** physically at the Day Care, the switch now processes **one interval at a time**: accumulate exactly one egg, collect it immediately through the same pipeline `collectDaycareSlot()` uses, set `breedingTested` for the result species, release both parents (including the `checkSpeciesCap()` call from "Universal Species-Cap Enforcement"), then — only if `researchMode` is still `true` — immediately select a new pair, continuing from the exact moment that egg hatched (not from "now"), and loop again against any remaining unconsumed elapsed time. Bounded by a 1000-iteration guard, the same spirit as `OFFLINE_LOOP_CAP` elsewhere. This mirrors what live, unthrottled ticking always did naturally — with the aide parked at the Day Care and `gameTick()` firing every second, `eggsQueued` reaches 1 and the very next tick collects and switches, one egg per pair, always. **Original bug:** the very first implementation instead let the whole elapsed backlog accumulate on the *current* pair before ever checking for a switch — so a single actual `gameTick()` firing after a browser-throttled background tab (which can go long stretches between firings) would let one species absorb dozens of eggs before switching, then switch at most once per call regardless of how much real time that call covered. Reported live: 32 eggs collected, only ~8 families newly researched from a single idle-offscreen session — confirmed root cause and confirmed fixed via direct testing (a 30-hatch-interval backlog in one call now spans 5 distinct species instead of 1). Unchecking the box takes effect the same way as before: it doesn't stop anything mid-cycle, it just prevents the next pair from being picked once the aide is next physically present for a switch.
...
- **v0.45.4 — iterates per (dexId, formName), not per dexId:** the loop's outer key is now every `state.formsDiscovered` entry (i.e. every form the player has actually encountered), not every `state.dexHistory` key. For each, the branch list to test comes from `getFormEvolutions(dexId, formName)` (new — filters `getEvolutions(dexId)` down to branches whose `fromFormName` matches this exact form) instead of every branch the dexId has across all its forms. See "Research Log — Per-Form Identity" below for why this replaced the old dexId-wide branch list.
...
#### Research Log — Per-Form Identity (SETTLED — v0.45.4, NEW)
- **A form is a different species for research purposes.** Completing base Growlithe's research page (evolution methods, abilities, gender ratio, breeding-tested) has no bearing on Hisuian Growlithe's — catching one for the first time starts its own page from scratch, exactly like catching any other new species would. This applies to every research field, not just evolution: `confirmedBranches`, `testedMethods`, `evolutionConfirmed`, `abilitiesObserved`, `gendersObserved`, `breedingTested`, `highestLevelObserved`, `firstSeen`, `firstCaught`, `knownSince`. `pokedex.js` already gives every form its own row with its own abilities/stats/gender ratio — this makes the research system agree with that, instead of quietly aggregating multiple forms' research into one shared record.
- **Does not change species/family identity elsewhere.** `getFamilyMembers()`, the Family/Species dex counts, `dexHistory`, and the Evolution Chain Visual's discovery gating are untouched — those already treat forms as distinct where it matters (each form is its own line/node) and continue to do so. This section is scoped to the research record only.
- **Root cause this replaces:** before this change, `state.researchLog[dexId]` was one shared record across every form of that dexId, but `getEvolutions(dexId)` (and every consumer of it) returned branches from *every* form indiscriminately, with no way to tell "the branches this specific form can actually confirm" from "every branch anything with this dexId has, anywhere." Two symptoms of this, both reported live:
  - **Growlithe showed Fire Stone as both ✓ Confirmed and ? Unknown simultaneously.** Growlithe has two Fire Stone branches sharing one dexId — base (`fromFormName:null`) and Hisuian (`fromFormName:'Hisuian'`). `professorAutoTestEvolutions()` only marked an item "fully tested" once *every* branch sharing that item resolved; the base branch confirmed fine, but the Hisuian branch could never resolve without an owned Hisuian Growlithe, so `testedMethods` never got 'Fire Stone' even after `confirmedBranches` already had it — and the two lists are built independently in `buildEvoMethodsHtml()`, so the same item rendered in both buckets at once.
  - **Vulpix's Ice Stone stayed at `?` forever**, with every other item correctly ruled out. Ice Stone's only branch requires `fromFormName:'Alolan'`; a player who only owns regular Vulpix can never produce a matching candidate, so the branch (and the item) could never resolve to either Confirmed or Ruled Out — a permanently unresolvable state with nothing distinguishing it from "haven't gotten to this yet."
- **Why per-form identity fixes both, structurally, not by patching either symptom:** once a research record belongs to one specific form, its branch list (`getFormEvolutions(dexId, formName)`) only ever contains branches whose `fromFormName` matches that exact form. Base Growlithe's own page never sees the Hisuian branch at all — it reaches "fully tested" the moment its own single Fire Stone branch resolves, regardless of whether Hisuian is ever owned (Hisuian isn't obtainable outside cheats — its own page simply doesn't exist until someone spawns one). Base Vulpix's own page never sees the Ice Stone branch either — with zero branches for that item on its own form, Ice Stone is ruled out immediately, cleanly, the same as every other inapplicable stone, instead of hanging at `?`. Alolan Vulpix and Hisuian Growlithe each get their own separate page (once owned) where their own item confirms normally.
- **Key format:** `getResearchKey(dexId, formName)` → `` `${dexId}|${formName||''}` `` — same convention as `state.formsDiscovered`'s key (see SAVE_VERSION 30→31). `getResearch(dexId, formName=null)` reads/creates through this key; every function that previously took a bare `dexId` (`recordSighting`, `checkMethodRuleOut`, `recordAbilityObserved`, `recordGenderObserved`, `recordNewSpecies`, `recordCapture`, `isSpeciesKnown`, `isDexPageComplete`, `speciesHasEvolveMethod`, `recordEvolution`) gains a trailing `formName=null` parameter, threaded from the individual Pokémon's own `p.formName` at every call site — `formName=null` is not a placeholder default that happens to work, it's the correct value for the large majority of species that only ever have one form.
- **Dex-complete IV bonus (`rollIVs()`) and `isDexPageComplete()`/`isSpeciesKnown()` are threaded the same way** — a newly-created individual's IV-roll advantage now checks its own form's completion, not a combined figure that could be satisfied partly by a form it isn't. `rollIVs(dexId, formName=null)`, called from `makePokemon()` with the already-resolved `entry.formName`.
- **Migration:** see SAVE_VERSION 31→32. Old aggregate research becomes the base form's record; any other form already owned starts fresh and self-heals on its own next natural trigger — no dedicated backfill code needed for that, since the same per-individual sweeps that already run on every load (ability/gender observation, method rule-out) simply create that form's record the first time they touch an individual of that form.
- **Data-side note (not a code fix):** this only works when a branch's `evolveItem` actually matches the item's real `itemId` slug, not its display name — `professorAutoTestEvolutions()`'s Path 2 always matched by slug, never by name. An audit of every `use-item` row in `evotree.js` against `items.js` found 3 rows using the item's display name instead (Vulpix and Growlithe's base rows: `"Fire Stone"` instead of `"fire-stone"`; Graveler's base row: `"Link Cable"` instead of `"link-cable"`) — corrected in the spreadsheet and `evotree.js` directly, no code involved. Without this fix, base Growlithe's own Fire Stone branch would still never match regardless of per-form scoping — see Resolved Bug Index.

#### Research State per Species (`state.researchLog[dexId+'|'+(formName||'')]`, v0.45.4 — see Per-Form Identity above)
Each **form** tracks:
...
- **v0.45.4 — per-form, not per-dexId:** `buildEvoMethodsHtml(dexId, formName=null)` reads the specific form's own research record. Two forms of the same dexId (e.g. base and Hisuian Growlithe) show independent X/N counts and independent ✓/?/Ruled Out buckets — see "Research Log — Per-Form Identity" above. Official Pokédex mode (spoiler-free reference, not gated by research) switches from listing every form's branches combined to only the currently-selected form's own (`dexSelectedFormName`), matching how the rest of that page already treats forms separately.
- **Residual edge case, display-layer guarded in v0.45.4:** `confirmedBranches` and `testedMethods` are still two independently-maintained lists, so a species with *two branches for the same item on the same form* could in principle have an item confirm on one branch while `professorAutoTestEvolutions()`'s "every branch must resolve" rule still keeps it out of `testedMethods` (the same structural gap that caused the original cross-form Growlithe bug). `buildEvoMethodsHtml()`'s Unknown-bucket filter now explicitly excludes anything already in the Confirmed bucket (`!confirmedMethods.includes(m)`), so the visible double-listing itself can no longer happen even in this same-form case — the item simply won't count toward "Ruled Out" or the N-tested total until every branch resolves, same as before. No such same-form multi-branch-per-item case exists in the data today.
...
- Two `EVO_TREE` columns, blank by default. `fromFormName` restricts a branch to individuals currently in a specific form (Wooper→Clodsire Paldean-only, Meowth's Galarian branch, Rockruff's Dusk branch). `requiredGender` restricts by gender (Kirlia→Gallade, Snorunt→Froslass, Burmy's two branches, Combee→Vespiquen, Salandit→Salazzle). Enforced in `professorAutoTestEvolutions()`'s item-candidate filter and in **both** of `checkEvolution()`'s and `applyEvolutionSilent()`'s Path 2 branch-qualifying filters (see v0.45.2 note below): `if((b.fromFormName||null)!==(p.formName||null)) return false;` / `if(branch.requiredGender && p.gender!==branch.requiredGender) return false;`
- **v0.45.2 fix — the two Path 2 filters had drifted.** `applyEvolutionSilent()` (offline/batch processing's counterpart to `checkEvolution()`, used specifically to avoid log spam over long away periods) carries its own separate copy of this branch filter, and that copy was missing the `fromFormName`/`requiredGender` lines entirely — an omission dating back to when this restriction was first added in v0.42, never mirrored into the silent path. Confirmed root cause of a base-form Meowth evolving into Perrserker at level 28 while an aide was away: with form unchecked, both the Persian branch and the Perrserker branch satisfy `level>=28` simultaneously, and `selectEvolutionBranch()`'s tie-break for a multi-candidate result is an unweighted random pick between them. Every species with a form-branch in EVO_TREE was equally exposed to this — Meowth was simply the one it happened to hit. A live evolution was never affected, since it always went through `checkEvolution()` instead, which had the filter correctly since v0.42. The two functions' Path 2 filters are now identical line-for-line; a Pokémon that already wrong-evolved before this fix isn't retroactively corrected.
...
- **v0.45 — same blind spot, second call site:** the Evolution Chain Visual's edge-resolution code had an equivalent gap independently of `checkEvolution()` — see "Per-Form Edge Resolution & Discovery Gating" under Evolution Chain Visual. A species migrated to the "clear the flat row, add an explicit `fromFormName:null` EVO_TREE row" pattern above fixes both the gameplay path and the chain visual at once, since both now key off the same `fromFormName` column the same way. Ten Kanto-era species (Rattata, Sandshrew, Vulpix, Diglett, Growlithe, Geodude, Graveler, Ponyta, Grimer, Voltorb) were migrated to this pattern as part of v0.45's data changes; migrating them is what fixes their cheat-spawned regional individuals' evolution outcome, not just their chain display (which the chain-visual fix alone already corrects, since it stops trusting the flat row's target once a same-form EVO_TREE row exists).
...
### Per-Form Edge Resolution & Discovery Gating (SETTLED — v0.45, supersedes v0.35 rule below)
- **Bug this replaces:** the v0.35 rule below decided "flat vs. EVO_TREE" per **species**, not per **form** — any EVO_TREE row for a dexId, regardless of whose form it actually belonged to, suppressed that species' flat edge entirely. For ~27 species where a regional/alt form's branch lives in EVO_TREE (Rattata, Sandshrew, Vulpix, Slowpoke, Meowth, Wooper, etc.), this meant the base form's real flat evolution was hidden and the alt-form's EVO_TREE edge was drawn in its place — reported live as "Rattata → Alolan Raticate."
- **New rule — edge selection is per node, keyed by `(dexId, formName)`, not per dexId alone:**
  - A node drawn as form F shows only EVO_TREE edges whose `fromFormName` equals F (`null`/blank = base form).
  - The flat `pokedex.js` edge is used for the base-form node **only when** it has a real `evolveMethod` **and** EVO_TREE has zero `fromFormName:null` rows for that dexId. A half-cleared flat row (method blank, `evolvesIntoId` still set, or vice versa) draws nothing — treated as absent, not as a dangling edge.
  - This scopes the old v0.35 "EVO_TREE wins outright" rule down to the matching form only — EVO_TREE having *a* row for the species no longer suppresses the *base* form's own flat row if that row is a genuinely different, unrelated form's branch.
- **Discovery gating, new axis orthogonal to "confirmed":** a node — base form included — only renders once that exact `(dexId, formName)` pair has been individually discovered, tracked by the new `state.formsDiscovered` record (see Versioning, SAVE_VERSION 31). This is separate from and stricter than the existing dexId-level `seenDexIds` used elsewhere (Pokédex grid, etc.) — `seenDexIds` doesn't distinguish forms, so it can't tell "regular Raichu seen" from "Alolan Raichu seen." Owning one form of a species never reveals a different form of that same species, in either direction — this is symmetric (regular Raichu doesn't reveal Alolan Raichu; Alolan Raichu doesn't reveal regular Raichu) and applies to branch targets too, so Pikachu's chain gives no hint that alternate Raichu forms exist until each is separately discovered.
- **Separate lines per source form, gated as a unit:** each distinct `fromFormName` among a species' EVO_TREE rows (plus the base flat edge, if eligible above) renders as its own row under the species' entry — same multi-root layout `renderEvolutionChainVisual()` already uses for true structural roots (Nidoran ♀/♂). A line appears at all only once **at least one form within that line** has been discovered; a line with zero discovered forms is omitted entirely — **no floating `?` placeholder for a wholly-undiscovered line.** This differs from the existing "unconfirmed branch" `???` treatment (still used, unchanged, for a discovered-but-not-yet-evolved node within a line that is already showing) — discovery gates whether the *line* appears at all; confirmation gates what a *visible* node inside that line looks like.
- **v0.45.1 — line ordering.** The base line (`fromFormName: null`) always renders first among a root's lines, regardless of which form was discovered first or which order EVO_TREE happens to list the rows in. Previously unsorted (`Set` iteration order = data order), which meant a regional/alt form's row could render ahead of the base line purely because it appeared earlier in `evotree.js` — reported live as an Alolan line showing above the regular one even with the regular form long since owned.
- **v0.45.3 fix — a non-evolving form could be silently dropped from the line list entirely.** `formNames` (which lines to render) was derived only from which forms had an outgoing edge — but `allMembers` (from `getFamilyMembers()`) deliberately collapses a species' forms down to one representative row per dexId, so a form that never evolves contributes zero edges and, under the old logic, got zero lines: not gated by discovery, just never considered at all. Confirmed on Farfetch'd — base Farfetch'd never evolves (no edge from it), Galarian Farfetch'd does (→ Sirfetch'd); only the Galarian line ever rendered, and the base line — Farfetch'd itself — never appeared regardless of research state. A species with only one form, or where every form evolves, never hit this, which is why Ditto and Mew rendered correctly the whole time on the exact same code path. Fixed by deriving `formNames` from the real forms present in `POKEDEX_DATA` for that dexId, unioned with any form appearing in an edge (belt-and-suspenders) — a non-evolving form now gets its own single-node line exactly like any other terminal species.
- **Worked example — Slowpoke:** regular Slowpoke (Slowbro/Slowking) and Galarian Slowpoke (Galarica Cuff/Wreath branches) are two separate lines under the same species entry. Owning only regular Slowpoke shows one line (Slowbro/Slowking, with the usual `?`/`???` treatment for whichever hasn't been confirmed yet); the Galarian line doesn't appear until a Galarian Slowpoke is itself discovered by any means.
- **Official Pokédex mode is unchanged** — everything renders regardless of discovery, exactly as it already does for the confirmed/unconfirmed distinction elsewhere in the chain.
- Display-only for the edge-resolution half of this rule; the discovery-gating half needs the new per-form record above (`state.formsDiscovered`), incremented alongside every existing `dexHistory` increment site via one shared helper (`recordFormDiscovered()`) — not scattered across all 8 call sites individually.
- **Implementation Note:** form keys must be normalized through `getPokemonEntry(dexId, formName)` before comparison — Nidoran's rows carry non-null `formName` values (`"Female"`/`"Male"`) while its EVO_TREE edges use `fromFormName:null`, which is a pre-existing, unrelated quirk of that species' data and must not be disturbed by this rule; Nidoran's existing two-root rendering stays exactly as-is.

### Two-Word Label Line Break (SETTLED — v0.45.1/v0.45.2)
- Any label rendered in the chain visual that contains a space forces a line break at the **first** space, unconditionally — not CSS `max-width` auto-wrap, which only kicked in once a label happened to overflow its box and was inconsistent across labels and container widths. Applies to:
  - **Evolution method/item arrow labels** (`evoArrowHtml()`, `evoArrowDownLabeledHtml()`) via a shared `forceTwoLineLabel(label)` helper — `"Moon Stone"` → `Moon`/`Stone`, `"Lv 7"` → `Lv`/`7`, `"Friendship (Night)"` → `Friendship`/`(Night)`. A label with more than one space (e.g. the Tyrogue special-case `"Lv20 · A>D"`) still only breaks once, at the first space, giving exactly two lines rather than one per space.
  - **Form-name node annotations** (`evoNodeHtml()`) — `"Dugtrio (Alolan)"` → `Dugtrio`/`(Alolan)`. This one does **not** reuse `forceTwoLineLabel()` — inserted as a direct break between the species name and the `(Form)` parenthetical instead, since a species name can itself contain a space (`"Mr. Mime"`), and a first-space split would wrongly break the name itself before ever reaching the form annotation.
- Fixes a reported case where a long item name (`"Moon Stone"`) pushed a family line's total width over the container, wrapping the next node down onto its own visual row — the same problem that made `"#51 Dugtrio (Alolan)"` overflow its 52px node box, since that label also had `white-space:nowrap` forcing it onto one line with no forced break to fall back on.

### Evolution Item Display — Slug vs. Display Name (SETTLED — v0.45.2)
- `evotree.js`'s `evolveItem` values are a genuine mix: the ten Kanto-era species added in v0.45 (Rattata, Sandshrew, etc.) hold a typed display string (`"Fire Stone"`), while most pre-existing branched rows hold a raw PokéAPI-style slug (`"leaf-stone"`, `"up-grade"`, `"dawn-stone"`, etc.). These slugs are exact matches for real `itemId`s in `items.js` — `getItem()` resolves them correctly — but `evoMethodLabel()`'s `use-item` branch previously only ever tried `getItem()` for a *numeric* `e.item`, so a string slug fell straight through to raw, unformatted display (`"leaf-stone"` shown verbatim in-game instead of `"Leaf Stone"`).
- **Fix:** `getItem(e.item)` is tried first regardless of type; only a string with no matching `itemId` falls back to raw display (covers this version's own typed values, e.g. `"Fire Stone"`, which correctly aren't in `items.js` under that exact casing). Verified `"up-grade"` resolves to the real `"Up-Grade"` (preserves its legitimate hyphen) rather than a naive space-substitution guess.

### Duplicate Branch on Legacy `evolvesIntoId` (SETTLED — v0.35, scope narrowed by the per-form rule above)
...
- **Implementation Note:** a species with both a legacy flat `evolvesIntoId` (`pokedex.js`) and `EVO_TREE` branch rows for the same dexId will draw the same evolution twice (plus any genuine additional branches) if both sources are counted — `EVO_TREE` must win outright, not merge, whenever it has any **same-form** rows for that species. As of v0.45 this comparison is scoped per `(dexId, formName)` — see the rule above — not per dexId alone.
- Display-only — `professorAutoTestEvolutions()` was never affected (already deduped via `alreadyConfirmed`). No SAVE_VERSION impact from this v0.35 rule itself (the v0.45 discovery-gating addition above is what carries the SAVE_VERSION bump).
...
- **v0.45.3 fix — this placeholder also requires the root itself to already be discovered.** Previously it didn't check that at all, so an evolution confirmed without ever personally owning its pre-evolution (e.g. a branch confirmed via research testing, but the base species never actually caught) rendered two leading "?"s stacked — one asking "might there be an even earlier unknown ancestor?", immediately in front of the root's own placeholder box for being unseen itself. Asking about an ancestor of something not yet known to exist doesn't make sense; the root's own unseen-placeholder already is the correct single indicator. Reported live on Cubone (Marowak owned, base Cubone never personally caught, branch confirmed via testing) — confirmed fixed: the redundant leading placeholder no longer appears while the root is undiscovered, and correctly re-engages once it is.
...
- **Gated (`requiresItem`) connections — per-aide, two lines (v0.45, doc corrected; supersedes v0.39.1 "most permissive" reading below):**
  - Two parallel dashed lines per gated connection, drawn with a small fixed pixel offset perpendicular to the connection — same fan-out approach as the existing same-location aide-marker offset (see Aide Roster & Hiring → Map). Carl Oak's line and Faraday's line, each **colored by that aide's own `trainerBag` only** — red (`#e63946`) if that specific aide doesn't own `requiresItem`, green (`#2ecc71`) once they do. No cross-aide check. `state.trainerUnlocks` — a dead stub from v0.19 that was never wired up (see Resolved Bug Index) — is removed from this check entirely; it was already inert.
  - **Orientation, derived from the connection's own geometry, not hardcoded per connection:** horizontal connection → Carl Oak's line on top, Faraday's on bottom. Vertical connection → Carl Oak's line on the left, Faraday's on the right. Diagonal connection → Carl Oak's line on the upper-left side of the pair, Faraday's on the lower-right.
  - **Before a second aide is hired:** a single centered dashed line, styled and positioned exactly as a gated connection rendered pre-v0.45 — no empty second slot is reserved or implied.
  - Ungated connections are unaffected — one solid blue line regardless of aide count.
  - Re-evaluated on every `renderMap()` call, same as before; in practice one-directional per aide (red→green only) since every gating item in the data is permanent once acquired.
  - **Cross-region stub lines (v0.45):** see "Region Tabs" below — a connection whose target lies in a different region (or lacks map coordinates) draws as a truncated stub rather than a full line to a destination node, using this same per-aide red/green coloring.
- *(Pre-v0.45, for reference: connections were colored by "any aide owns it" — `state.aides.some(a=>a.trainerBag[requiresItem]>0) || state.trainerUnlocks[requiresItem]` — a most-permissive/informative-for-a-shared-map reading. Superseded because `buildTravelPath()`/`getReachableDiscoveredLocations()` have always gated actual travel per-aide, so the shared line was cosmetically optimistic about what a specific aide could actually do.)*
...
### Map Zoom & Pan (SETTLED — v0.45, NEW)
- Zoom/pan is layered on top of the existing auto-fit `viewBox` computation — `renderMap()` still computes the fit-all `viewBox` exactly as before; a separate scale+translate transform is applied on top of that base, scoped entirely to `#map-container`/`#map-svg`. No other panel (header, tab bar, `#map-detail`) is touched by this feature.
- **Gestures:** pinch-to-zoom and one-finger drag-to-pan on touch; scroll-wheel (and trackpad-pinch, reported by browsers as `wheel`+`ctrlKey`) to zoom, click-drag to pan, on desktop. Both sets attach to `#map-container` only.
- **Bounds:** minimum zoom = the current fit-all (can't zoom out past the whole discovered region's extent). Maximum zoom = **6× fit-all**.
- **Reset triggers (three, all snap to fit-all — no zoom state is ever preserved across these):**
  1. Leaving and re-entering the Map tab.
  2. Any `renderMap()` redraw triggered by a newly discovered location.
  3. Switching region tabs (see "Region Tabs" below).
- **Hit-testing unaffected:** `handleMapClick(event)` and node click targets resolve correctly at any zoom level — the transform applies to the SVG's coordinate space; existing element coordinates and click handlers need no change.
- Display/interaction-only — no `state` schema change, no SAVE_VERSION impact.

### Region Tabs (SETTLED — v0.45, NEW)
- **Motivation:** `renderMap()`'s viewBox auto-fit previously spanned every discovered location regardless of `region` (a field that already existed on `LOCATIONS` but was previously display-only, shown in `showMapDetail()` and nowhere else). Once Sevii Islands locations receive real `mapCol`/`mapRow` values, a single shared grid would stretch across two geographically separate landmasses — the Kanto view would distort to accommodate Sevii coordinates far outside its own extent.
- **Tabs are auto-derived, never hardcoded:** one tab per distinct `region` value present in `LOCATIONS`, ordered by first appearance in that file. A future third region needs a data change only — no code change.
- **Enabled/disabled:** a tab is enabled once `state.discoveredLocations` contains at least one location in that region; otherwise it renders grayed out and non-interactive. Tabs for every region always render, discovered or not.
- **Default tab on opening Map:** whichever region contains the most recently active aide's current location; falls back to the first tab (by file order) if that can't be determined (e.g. immediately post-load).
- **Per-region viewBox scoping:** `renderMap()`'s min/max col/row computation filters `visibleKeys` to the active tab's `region` before computing the fit-all extent — this is the actual fix for the cross-region-stretch problem above. Sevii's grid never factors into Kanto's fit, and vice versa.
- **Cross-region connections draw as edge stubs, not dropped:** previously, a connection whose target lacked `mapCol`/`mapRow` (Sevii's current state) was silently skipped by the existing "both endpoints must have coordinates" guard. As of v0.45, a connection whose two discovered endpoints sit in **different regions** — or whose target has no map coordinates yet — draws as a short dotted stub extending from the source node past the current grid's edge, in the direction away from the grid's geometric center (computed from the grid extent, not hardcoded per connection — this naturally points the S.S. Anne stub "south," since Vermillion City/S.S. Anne already sit near the bottom of Kanto's current extent, without that direction being special-cased). No destination node is drawn. Colored red/green by the same live per-aide ownership check as any other gated connection (see Map System → Connections). This rule is symmetric — Sevii's own map will show a matching stub back toward Kanto once Sevii's grid has real coordinates.
- **Worked example — S.S. Anne:** `sSAnne → oneIsland` (gated on `sSTicket`) is the first connection this applies to. `sSAnne` is a real, positioned Kanto node; `oneIsland` (Sevii Islands) currently has `mapCol`/`mapRow` both `null`. Before v0.45 this edge was invisible. As of v0.45 it renders as a red/green dotted stub trailing off the bottom of the Kanto map.
- **Data note (not a code gap):** six locations (Two Island, Cape Brink, Three Isle Port, Three Island, Bond Bridge, Berry Forest) carry `region: null` in the live data as of v0.45 — they belong to no tab until tagged. Flagged under Outstanding Data Tasks below; does not block this feature, since the tab system degrades gracefully (a `null`-region location is simply unreachable via any tab until tagged).
- No `state` schema change — `region` already existed on location data; this is new read-side logic only. No SAVE_VERSION impact.
...
- `state.trainerUnlocks` — **resolved v0.45.** Had existed as a dead, never-wired-up stub since this v0.19 note was first written; confirmed to have no live writer anywhere in the code. Removed from every place still reading it — the map's gated-connection ownership check (Connections under Map System), `buildTravelPath()`, the reachability BFS behind the mission-destination picker, and `getMissingItemGatesForLocation()`. Its "relationship to per-aide inventory" is retroactively "none — it was inert."
...
| v0.45 | 7 | Evolution Chain Visual (per-form edge resolution + discovery gating), Data Architecture/spreadsheet (family-number renumbering + `FullFetcher.html` derivation logic), Map System (per-aide gated-connection lines, Map Zoom & Pan, Region Tabs), Mission Modal — Wander Mode (third "random" metric) |
| v0.45.1 | 3 | Evolution Chain Visual (`formsDiscovered` load-restore bug + self-heal, line-ordering fix, Two-Word Label Line Break for arrow/method labels) |
| v0.45.2 | 3 | Evolution Chain Visual (Two-Word Label Line Break extended to form-name node annotations, Evolution Item Display slug-vs-name fix), Branching Evolutions (`applyEvolutionSilent()` `fromFormName`/`requiredGender` parity fix) |
| v0.45.3 | 3 | Evolution Chain Visual (non-evolving-form line-drop fix, redundant breeding-placeholder fix), Day Care Research Mode (per-egg switch loop, replacing the one-switch-per-call cap) |
| v0.45.4 | 4 | Research & Pokédex (Research Log — Per-Form Identity, replacing the shared-per-dexId research record; Three-Bucket UI now per-form), Tech Stack (canonical-current-version note), data-only `evotree.js` slug fix (Vulpix/Growlithe/Graveler base rows), Resolved Bug Index |

`SAVE_VERSION`: 27→28 (v0.40), stays on 28 (v0.41–v0.43), reaches 30 across two bumps (v0.44), no bump (v0.44.1–v0.44.4), 30→31 (v0.45), no bump (v0.45.1–v0.45.3), 31→32 (v0.45.4).
...
- **Six locations with `region: null`** (Two Island, Cape Brink, Three Isle Port, Three Island, Bond Bridge, Berry Forest) — flagged during v0.45's Region Tabs work. They read as unfinished Sevii Islands entries by name, but aren't tagged, so they belong to no region tab as of v0.45. Not code-blocking; tag them (presumably `"Sevii Islands"`) whenever convenient.
