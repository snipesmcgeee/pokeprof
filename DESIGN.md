...
- **Redirect logic:** if any reachable location other than the current target is *strictly* lower, rebuild `state.travelPath` from the aide's current location to the new target (`buildTravelPath()`) and reset `travelPathIndex`. If the current target is still tied for lowest, it's kept — no redirect purely from equal rankings, avoiding needless churn.
- **Why this doesn't thrash:** sightings only accumulate at the aide's *current* location — an idle location's count can't change while the aide is elsewhere, so two idle locations can never ping-pong the aide back and forth. A redirect only happens when the current waypoint's own count climbs high enough that some other already-known location becomes definitively lower.
