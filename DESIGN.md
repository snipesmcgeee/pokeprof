...
### 13. Gym Encounters Excluded at Mission Origin (Wander Deadlock Fix)

**Bug** (reported post-build, fixed within v0.44): a Wander aide could get permanently stuck cycling through the same location forever, requiring manual intervention. Root cause: Wander routes toward whichever reachable location currently has the lowest encounter/catch count, and the path there can pass through other locations as waypoints — encounters (including a checked Gym Battle method, per v0.40's "gym battles available at any location the aide occupies") roll at any waypoint being dwelled at, not just the final destination. If a gym loss (or a tough wild encounter further along the same route) wipes the whole party, `endMission('faint')` recalls the aide to `lastHealLocation` and rebuilds the same path toward the same Wander target — and if that heal location is also where the current mission's `missionOrigin` sits (very commonly true: a player manually walks an aide somewhere, then turns Wander on from there), the very next dwell at that same spot immediately re-rolls the same gym fight before the aide ever gets away again. Confirmed via simulation: an aide dispatched into Wander from Viridian City with Gym Battle checked there never left — every wipe sent it straight back into another attempt at the same gym, indefinitely.

**Fix**: `rollEncounter()`'s gym-eligibility check now excludes the aide's own `missionOrigin` — `getGymTrainerIdsAtLocation(locId)` is skipped entirely (empty list) whenever `locId===aide.missionOrigin`. `missionOrigin` is set once at `confirmMission()` and never updated by Wander's own redirects, so it reliably identifies "wherever this dispatch actually started from," not just wherever the aide currently happens to be. Gym battles remain fully available at every other location the aide passes through or is dispatched to, unchanged from v0.40 — this only exempts the starting point, closing off the specific recall-and-repeat loop without touching the general "fight gyms while wandering" feature the exclusion sits inside.
