...
- **Verification:** 91 automated assertions across 11 jsdom-based test scripts (fresh boot, full mission flow, dual-aide `gameTick()` stress test, full save→reload round-trip, map rendering with both aides at the same location, full hire-flow, a complete realistic session end-to-end, a dedicated battle-engine test confirming Faraday's gym win credits his own `trainerBag` specifically not Carl Oak's, a CSS-layout regression test, and — v0.39.3 — a dedicated test assigning a box Pokémon to Faraday specifically, plus a migration test using a hand-crafted fake v26 two-aide save to confirm no data loss when `dex` moved back to shared).
