...
### 13. Gym Battles Excluded at the Aide's Current Base + No-Prefs Default Respects Earned-Badge Status (Wander Deadlock Fix)

**Bug** (reported post-build, fixed within v0.44, in two rounds): a Wander aide could get permanently stuck cycling through the same location forever, requiring manual intervention — reported recurring at each new city reached (Cerulean, then Saffron, then Vermillion), "fixed" temporarily each time only by manually checking that city's Gym Battle box and re-dispatching.

**Root cause, part 1**: Wander routes toward whichever reachable location currently has the lowest encounter/catch count, and the path there can pass through other locations as waypoints — encounters (including Gym Battle, per v0.40's "gym battles available at any location the aide occupies") roll at any waypoint being dwelled at, not just the final destination. A party wipe (gym loss or a tough wild encounter) triggers `endMission('faint')`, which recalls the aide to `lastHealLocation` and rebuilds the same path toward the same Wander target. The first round of this fix excluded gym battles only at `aide.missionOrigin` — but `missionOrigin` is set once at `confirmMission()` and never updated by Wander's own redirects, while `lastHealLocation` is what actually tracks the aide's current progress (updated in `arriveAtLocation()` every time they reach a `heals:true` city — Pewter → Cerulean → Saffron → Vermillion in sequence). Excluding only the stale `missionOrigin` left every new city the aide progressed to completely unprotected until the player manually re-dispatched from it — which is exactly what "checking the box and hitting Dispatch" did: it reset `missionOrigin` to wherever they were standing, coincidentally protecting that one spot until the next city repeated the same problem.
...
**Fix**: `rollEncounter()`'s gym-eligibility check now excludes gym at `locId===aide.missionOrigin || locId===aide.lastHealLocation` — covering both the original dispatch point and wherever the aide's progress currently sits. `pickMethodForLocation()` (now taking a `trainerBag` param) mirrors the checkbox's own default in its no-prefs fallback: gym only joins the random pool if `hasGymBadge()` is already true for that trainer, exactly matching what the UI would show as checked by default. Once a location's prefs are explicitly saved (player has genuinely opened and configured that city's panel), gym battling works exactly as before, unaffected by either change. Verified via a 3000-tick simulation: a fresh Wander dispatch from Pallet Town, with default settings throughout (no location's checkbox ever touched), now progresses through 14 discovered locations and levels the party to the tier-1 cap, instead of stalling.
...
- The item #13 deadlock-fix exclusions (`missionOrigin`/`lastHealLocation`) apply unconditionally regardless of this preference — choosing YES makes gym battles more common everywhere else, but never reintroduces the recall-and-repeat loop at the aide's own base.
