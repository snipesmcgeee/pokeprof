...
- Current save version: `26` (bumped in v0.39.1 for the `state.aides[]` restructure — Carl Oak's singular `party`/`trainerBag`/`currentLocation`/mission-and-travel fields wrapped into `aides[0]` on migration; unchanged in v0.39.2, which was battle-engine/logic fixes only, no new state fields). v0.38→v0.39 made no schema change (Silph Tower was fully reverted in v0.39.1 anyway, and was itself runtime-only/non-persisted).
...
- **Verification:** 64 automated assertions across 8 jsdom-based test scripts (fresh boot, full mission flow, dual-aide `gameTick()` stress test, full save→reload round-trip, map rendering with both aides at the same location, full hire-flow, a complete realistic session end-to-end, and — v0.39.2 — a dedicated battle-engine test confirming Faraday's gym win credits his own `trainerBag` specifically, not Carl Oak's, plus 50 forced-collision `gameTick()` calls with no throw).
